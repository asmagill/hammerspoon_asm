...
static int push<moduleTyoe>(lua_State *L, id obj) {
    <moduleTyoe> *value = obj;
    value.selfRefCount++ ;
    void** valuePtr = lua_newuserdata(L, sizeof(<moduleTyoe> *));
...
id to<moduleTyoe>FromLua(lua_State *L, int idx) {
    LuaSkin *skin = [LuaSkin shared] ;
    <moduleTyoe> *value ;
    if (luaL_testudata(L, idx, USERDATA_TAG)) {
        value = get_objectFromUserdata(__bridge <moduleTyoe>, L, idx, USERDATA_TAG) ;
    } else {
...
static int userdata_tostring(lua_State* L) {
    LuaSkin *skin = [LuaSkin shared] ;
//     <moduleTyoe> *obj = [skin luaObjectAtIndex:1 toClass:"<moduleTyoe>"] ;
...
// so use luaL_testudata before the macro causes a lua error
    if (luaL_testudata(L, 1, USERDATA_TAG) && luaL_testudata(L, 2, USERDATA_TAG)) {
        LuaSkin *skin = [LuaSkin shared] ;
        <moduleTyoe> *obj1 = [skin luaObjectAtIndex:1 toClass:"<moduleTyoe>"] ;
        <moduleTyoe> *obj2 = [skin luaObjectAtIndex:2 toClass:"<moduleTyoe>"] ;
...
static int userdata_gc(lua_State* L) {
    <moduleTyoe> *obj = get_objectFromUserdata(__bridge_transfer <moduleTyoe>, L, 1, USERDATA_TAG) ;
    if (obj) {
        obj.selfRefCount-- ;
...
int luaopen_hs__asm_dnssd_service(lua_State* __unused L) {
    ...
    [skin registerPushNSHelper:push<moduleTyoe>         forClass:"<moduleTyoe>"];
    [skin registerLuaObjectHelper:to<moduleTyoe>FromLua forClass:"<moduleTyoe>"
