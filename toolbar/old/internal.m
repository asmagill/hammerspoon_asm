...
@property (readonly) CGFloat       fieldHeight ;
@property (readonly) CGFloat       fieldWidth ;
@end
...
- (void)modifyToolbarItem:(NSToolbarItem *)item fromTableAtIndex:(int)idx thatIsNew:(BOOL)newItem {
    ...
    if ((newItem || [item isEqualTo:[_itemDictionary objectForKey:identifier]])
        ...
        if (lua_getfield(L, idx, "enabled") == LUA_TBOOLEAN) {
            _enabledDictionary[identifier] = @((BOOL)lua_toboolean(L, -1)) ;
        } else if (newItem) {
        ...
        if ((lua_getfield(L, idx, "fn") == LUA_TFUNCTION) || (lua_isboolean(L, -1) && !lua_toboolean(L, -1))) {
            if ([[_fnRefDictionary objectForKey:identifier] intValue] != LUA_NOREF) {
                _fnRefDictionary[identifier] = @([skin luaUnref:refTable ref:[[_fnRefDictionary objectForKey:identifier] intValue]]) ;
            }
        ...
        lua_pop(L, 1) ;
        if ((lua_getfield(L, idx, "searchfield") == LUA_TBOOLEAN) && lua_toboolean(L, -1)) {
            ASMToolbarSearchField *searchField = [[ASMToolbarSearchField alloc] init];
            searchField.toolbarItem = item ;
            searchField.action      = @selector(performCallback:) ;
...
// for grouped sets, the palette label *must* be set or unset in sync with label, otherwise it only shows some of the individual labels... so simpler to just forget that there are actually two labels. Very few will likely care/notice anyways.
        item.label        = [skin toNSObjectAtIndex:-1] ;
        item.paletteLabel = [skin toNSObjectAtIndex:-1] ;
...
#pragma clang diagnostic pop
    ...
    if (lua_getfield(L, idx, "tooltip") == LUA_TSTRING) {
        item.toolTip = [skin toNSObjectAtIndex:-1] ;
    } else if ((lua_type(L, -1) == LUA_TBOOLEAN) && !lua_toboolean(L, -1)) {
        item.toolTip = nil ;
    }
    lua_pop(L, 1) ;
    if ((lua_getfield(L, idx, "priority") == LUA_TNUMBER) && lua_isinteger(L, -1)) {
        item.visibilityPriority = lua_tointeger(L, -1) ;
    }
    lua_pop(L, 1) ;
    if ((lua_getfield(L, idx, "tag") == LUA_TNUMBER) && lua_isinteger(L, -1)) {
        item.tag = lua_tointeger(L, -1) ;
    }
    lua_pop(L, 1) ;

    if ((lua_getfield(L, idx, "image") == LUA_TUSERDATA) && luaL_checkudata(L, -1, "hs.image")) {
        item.image = [skin toNSObjectAtIndex:-1] ;
    } else if ((lua_type(L, -1) == LUA_TBOOLEAN) && !lua_toboolean(L, -1)) {
        item.image = nil ;
    } else if (newItem && ![item isKindOfClass:[NSToolbarItemGroup class]]) {
        item.image = [NSImage imageNamed:NSImageNameStatusNone] ;
    }
    lua_pop(L, 1) ;

    // However, the following searchfield items can be modified after creation of the searchfield, so do them here to reduce replicated code...
    if ([item.view isKindOfClass:[ASMToolbarSearchField class]]) {
        ASMToolbarSearchField *searchField = (ASMToolbarSearchField *)item.view ;
        CGFloat minWidth = searchField.fieldWidth ;
        CGFloat maxWidth = searchField.fieldWidth ;
        BOOL sizeChanged = NO ;
        if (lua_getfield(L, idx, "searchMinWidth") == LUA_TNUMBER) {
            // make sure it's not less than field default
            minWidth = MAX(minWidth, lua_tonumber(L, -1)) ;
            sizeChanged = YES ;
        }
        lua_pop(L, 1) ;
        if (lua_getfield(L, idx, "searchMaxWidth") == LUA_TNUMBER) {
            // make sure it's not less than field default
            maxWidth = MAX(maxWidth, lua_tonumber(L, -1)) ;
            sizeChanged = YES ;
        }
        lua_pop(L, 1) ;
        // make sure it's not less than the minimum we've accepted
        maxWidth = MAX(maxWidth, minWidth) ;
        if (sizeChanged) {
            NSRect searchFieldRect = [searchField frame] ;
            searchFieldRect.size.width = maxWidth ;
            searchField.frame = searchFieldRect ;
            item.minSize = NSMakeSize(minWidth, searchField.fieldHeight) ;
            item.maxSize = NSMakeSize(maxWidth, searchField.fieldHeight) ;
        }


        if ((lua_getfield(L, idx, "searchText") == LUA_TSTRING) || lua_isnumber(L, -1)) {
            luaL_checkstring(L, -1) ;
            searchField.stringValue = [skin toNSObjectAtIndex:-1] ;
        }
        lua_pop(L, 1) ;

        if ((lua_getfield(L, idx, "searchPredefinedSearches") == LUA_TTABLE) || ((lua_type(L, -1) == LUA_TBOOLEAN && !lua_toboolean(L, -1)))) {
            ...
            if (lua_type(L, -1) == LUA_TTABLE) {
                ...
                int position = 0 ;
                while (position < luaL_len(L, tableIndex)) {
                    position++ ; // lua tables start at 1
                    lua_rawgeti(L, tableIndex, position) ;
                    luaL_tolstring(L, -1, NULL) ; // convert whatever to string with __tostring, if available
                    NSString *itemTitle = [skin toNSObjectAtIndex:-1] ;
                    lua_pop(L, 2) ; // the item from the table and the converted-to-string version of it
                    NSMenuItem* newMenuItem = [[NSMenuItem alloc] initWithTitle:itemTitle
                                                                         action:@selector(searchCallback:)
                                                                  keyEquivalent:@""] ;
                    newMenuItem.target = searchField ;
            ...
            ((NSSearchFieldCell *)searchField.cell).searchMenuTemplate = searchMenu ;
        }
        lua_pop(L, 1) ;

        if ((lua_getfield(L, idx, "searchHistoryLimit") == LUA_TNUMBER) && lua_isinteger(L, -1)) {
            ((NSSearchFieldCell *)searchField.cell).maximumRecents = lua_tointeger(L, -1) ;
        }
        lua_pop(L, 1) ;

        if (lua_getfield(L, idx, "searchHistory") == LUA_TTABLE) {
            NSArray *table = [skin toNSObjectAtIndex:-1] ;
            if ([table isKindOfClass:[NSArray class]]) {
                BOOL allGood = YES ;
                for (NSString *item in table) {
                ...
                if (allGood) {
                    ((NSSearchFieldCell *)searchField.cell).recentSearches = table ;
                } else {
                    [skin logWarn:[NSString stringWithFormat:@"%s:search history array must contain only strings", USERDATA_TAG]] ;
                }
            } else {
                [skin logWarn:[NSString stringWithFormat:@"%s:search history must be an array", USERDATA_TAG]] ;
            }
        }
        lua_pop(L, 1) ;

        if ((lua_getfield(L, idx, "searchHistoryAutoSaveName") == LUA_TSTRING)  || ((lua_type(L, -1) == LUA_TBOOLEAN && !lua_toboolean(L, -1)))) {
            NSString *newName  = nil ;
            if (lua_type(L, -1) == LUA_TSTRING) newName = [skin toNSObjectAtIndex:-1] ;
            ((NSSearchFieldCell *)searchField.cell).recentsAutosaveName = newName ;
        }
        lua_pop(L, 1) ;
    }
...
- (BOOL)addToolbarItemGroupAtIndex:(int)idx {
    ...
    if (!isGood) return NO ;
    _groupings[identifier] = subItems ;

    _itemDictionary[identifier] = groupItem ;
...
- (BOOL)addToolbarItemAtIndex:(int)idx asPrimaryItem:(BOOL)isPrimary {
    ...
    if (![builtinToolbarItems containsObject:identifier]) {
        ...
        _itemDictionary[identifier] = toolbarItem ;
        if (selectable) [_selectableIdentifiers addObject:identifier] ;
    }
    ...
    if ([_groupings objectForKey:itemIdentifier]) {
        ...
        for (NSString *theId in [_groupings objectForKey:itemIdentifier]) {
            ...
            if (flag) {
                ...
                if ([newItem.view isKindOfClass:[ASMToolbarSearchField class]]) {
                    // we need to copy the ASMToolbarSearchField so that it can be given the correct toolbar item for use in the callback
                    ASMToolbarSearchField *searchField = [(ASMToolbarSearchField *)newItem.view copy] ;
                    searchField.toolbarItem = newItem ;
                    searchField.target      = toolbar ;
                    newItem.view = searchField;
                }
            }
        ...
        if (flag) {
            ...
            if ([item.view isKindOfClass:[ASMToolbarSearchField class]]) {
                // we need to copy the ASMToolbarSearchField so that it can be given the correct toolbar item for use in the callback
                ASMToolbarSearchField *searchField = [(ASMToolbarSearchField *)item.view copy] ;
                searchField.toolbarItem = item ;
                searchField.target      = toolbar ;
                item.view = searchField;
            }
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _fieldHeight     = cellFrame.size.height ;
        _fieldWidth      = cellFrame.size.width ;
        _minWidth        = _fieldWidth ;
        _maxWidth        = _fieldWidth ;
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    _minWidth    = self.minWidth ;
    _maxWidth    = self.maxWidth ;
    _fieldHeight = self.fieldHeight ;
    _fieldWidth  = self.fieldWidth ;
    copy.cell    = self.cell ;
...
static int attachToolbar(lua_State *L) {
    ...
    if (newToolbar) {
        if (newToolbar.windowUsingToolbar) newToolbar.windowUsingToolbar.toolbar = nil ;
...
static int userdata_gc(lua_State* L) {
    ...
    if (obj) {
        ...
        if (obj.windowUsingToolbar && [[obj.windowUsingToolbar toolbar] isEqualTo:obj])
            obj.windowUsingToolbar.toolbar = nil ;
