...
static const char *USERDATA_TAG = "hs._asm.sharing" ;
...
@interface ASMSharingService : NSObject <NSSharingServiceDelegate>
@property NSSharingService *sharingService ;
@property int              callbackRef ;
@property int              selfRefCount ;
@end

@implementation ASMSharingService
...
/// hs._asm.sharing.newShare(type) -> sharingObject
/// Constructor
...
static int sharing_new(lua_State *L) {
    ...
    ASMSharingService *wrapper = [[ASMSharingService alloc] initWithService:[skin toNSObjectAtIndex:1]] ;
...
/// hs._asm.sharing.shareTypesFor(items) -> identifiersTable
/// Function
...
/// hs._asm.sharing.URL(URL, [fileURL]) -> table
/// Function
...
/// hs._asm.sharing:shareItems(items) -> sharingObject
/// Method
...
static int sharing_performWith(lua_State *L) {
    ...
    ASMSharingService *wrapper = [skin toNSObjectAtIndex:1] ;
...
/// hs._asm.sharing:canShareItems(items) -> boolean
/// Method
...
/// hs._asm.sharing:callback(function | nil) -> sharingObject
/// Method
...
/// hs._asm.sharing:recipients([recipients]) -> current value | sharingObject
/// Method
...
/// hs._asm.sharing:subject([subject]) -> current value | sharingObject
/// Method
/// Get or set the subject to be used when the sharing service performs its sharing method.
///
/// Parameters:
...
/// hs._asm.sharing:attachments() -> table | nil
/// Method
...
/// hs._asm.sharing:accountName() -> string | nil
/// Method
...
/// hs._asm.sharing:messageBody() -> string | nil
/// Method
...
/// hs._asm.sharing:serviceName() -> string
/// Method
...
/// hs._asm.sharing:permanentLink() -> URL table | nil
/// Method
...
/// hs._asm.sharing:alternateImage() -> hs.image object | nil
/// Method
...
/// hs._asm.sharing:image() -> hs.image object | nil
/// Method
...
static int pushASMSharingService(lua_State *L, id obj) {
    ASMSharingService *value = obj;
    value.selfRefCount++ ;
    void** valuePtr = lua_newuserdata(L, sizeof(ASMSharingService *));
...
id toASMSharingServiceFromLua(lua_State *L, int idx) {
    LuaSkin *skin = [LuaSkin shared] ;
    ASMSharingService *value ;
    if (luaL_testudata(L, idx, USERDATA_TAG)) {
        value = get_objectFromUserdata(__bridge ASMSharingService, L, idx, USERDATA_TAG) ;
    } else {
...
static int userdata_tostring(lua_State* L) {
    LuaSkin *skin = [LuaSkin shared] ;
    ASMSharingService *obj = [skin luaObjectAtIndex:1 toClass:"ASMSharingService"] ;
...
// so use luaL_testudata before the macro causes a lua error
    if (luaL_testudata(L, 1, USERDATA_TAG) && luaL_testudata(L, 2, USERDATA_TAG)) {
        LuaSkin *skin = [LuaSkin shared] ;
        ASMSharingService *obj1 = [skin luaObjectAtIndex:1 toClass:"ASMSharingService"] ;
        ASMSharingService *obj2 = [skin luaObjectAtIndex:2 toClass:"ASMSharingService"] ;
...
static int userdata_gc(lua_State* L) {
    ASMSharingService *obj = get_objectFromUserdata(__bridge_transfer ASMSharingService, L, 1, USERDATA_TAG) ;
    if (obj) {
        obj.selfRefCount-- ;
...
int luaopen_hs__asm_sharing_internal(lua_State *L) {
    LuaSkin *skin = [LuaSkin shared] ;
    ...
    [skin registerPushNSHelper:pushASMSharingService         forClass:"ASMSharingService"];
    [skin registerLuaObjectHelper:toASMSharingServiceFromLua forClass:"ASMSharingService"
