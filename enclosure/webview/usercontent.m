...
- (void)userContentController:(__unused WKUserContentController *)userContentController
    ...
    if ([message.name isEqualToString:self.name] && self.userContentCallback != LUA_NOREF) {
        ...
        if (![skin protectedCallAndTraceback:1 nresults:0]) {
            ...
            [skin logError:[NSString stringWithFormat:@"%s callback error: %s", USERDATA_UCC_TAG, errorMsg]];
            lua_pop([skin L], 1) ;
        }
    }
}
...
/// hs._asm.enclosure.webview.usercontent.new(name) -> usercontentControllerObject
/// Constructor
...
///
/// Notes:
///  * This object should be provided as the final argument to the `hs._asm.enclosure.webview.new` constructor in order to tie the webview to this content controller.  All new windows which are created from this parent webview will also use this controller.
///  * See `hs._asm.enclosure.webview.usercontent:setCallback` for more information about the message port.
...
/// hs._asm.enclosure.webview.usercontent:injectScript(scriptTable) -> usercontentControllerObject
/// Method
...
/// hs._asm.enclosure.webview.usercontent:userScripts() -> array
/// Method
...
/// hs._asm.enclosure.webview.usercontent:removeAllScripts() -> usercontentControllerObject
/// Method
...
///  * the usercontentControllerObject
/// Notes:
///  * The WKUserContentController class only allows for removing all scripts.  If you need finer control, make a copy of the current scripts with `hs._asm.enclosure.webview.usercontent.userScripts()` first so you can recreate the scripts you want to keep.
...
/// hs._asm.enclosure.webview.usercontent:setCallback(fn) -> usercontentControllerObject
/// Method
...
// TODO: we should "overwrite" this in the hs.webview wrapper
//       [skin pushNSObject:(HSWebViewWindow *)((HSWebViewView *)message.webView).window] ;
//           lua_setfield(L, -2, "webView") ;
      [skin pushNSObject:message.webView] ;   lua_setfield(L, -2, "webView") ;
    return 1 ;
}
...
int luaopen_hs__asm_enclosure_webview_usercontent(lua_State* L) {
    LuaSkin *skin = [LuaSkin shared] ;
