...
/// === hs._asm.enclosure.webview.datastore ===
///
/// Provides methods to list and purge the various types of data used by websites visited with `hs._asm.enclosure.webview`.
///
/// This module is only available under OS X 10.11 and later.
///
/// This module allows you to list and selectively purge the types of data stored locally for the websites visited with the `hs._asm.enclosure.webview` module.  It also adds support for non-persistent datastores to `hs._asm.enclosure.webview` (private browsing) and allows a non-persistent datastore to be shared among multiple instances of `hs._asm.enclosure.webview` objects.
///
/// The datastore for a webview contains various types of data including cookies, disk and memory caches, and persistent data such as WebSQL, IndexedDB databases, and local storage.  You can use methods in this module to selectively or completely purge the common datastore (used by all Hammerspoon `hs._asm.enclosure.webview` instances that do not use a non-persistent datastore).
...
/// hs._asm.enclosure.webview.datastore.websiteDataTypes() -> table
/// Function
...
/// hs._asm.enclosure.webview.datastore.default() -> datastoreObject
/// Constructor
/// Returns an object representing the default datastore for Hammerspoon `hs._asm.enclosure.webview` instances.
///
/// Parameters:
///  * None
///
...
///
/// Notes:
///  * this is the datastore used unless otherwise specified when creating an `hs._asm.enclosure.webview` instance.
...
/// hs._asm.enclosure.webview.datastore.newPrivate() -> datastoreObject
/// Constructor
/// Returns an object representing a newly created non-persistent (private) datastore for use with a Hammerspoon `hs._asm.enclosure.webview` instance.
///
/// Parameters:
///  * None
///
...
///
/// Notes:
///  * The datastore represented by this object will be initially empty.  You can use this function to create a non-persistent datastore that you wish to share among multiple `hs._asm.enclosure.webview` instances.  Once a datastore is created, you assign it to a `hs._asm.enclosure.webview` instance by including the `datastore` key in the `hs.webvew.new` constructor's preferences table and setting it equal to this key.  All webview instances created with this datastore object will share web caches, cookies, etc. but will still be isolated from the default datastore and it will be purged from memory when all of the webviews are deleted, or Hammerspoon is restarted.
///
///  * Using the `datastore` key in the webview's constructor differs from the `private` key -- use of the `private` key will override the `datastore` key and will create a separate non-persistent datastore for the webview instance.  See `hs._asm.enclosure.webview.new` for more information.
...
/// hs._asm.enclosure.webview.datastore.fromWebview(webview) -> datastoreObject
/// Constructor
/// Returns an object representing the datastore for the specified `hs._asm.enclosure.webview` instance.
///
/// Parameters:
///  * `webview` - an `hs._asm.enclosure.webview` instance (webviewObject)
///
/// Returns:
///  * a datastoreObject
///
/// Notes:
///  * When running on a system with OS X 10.11 or later, this method will also be added to the metatable for `hs._asm.enclosure.webview` objects so that you can retrieve a webview's datastore with `hs._asm.enclosure.webview:datastore()`.
///
...
static int datastore_fromWebview(lua_State *L) {
    ...
    HSWebViewView          *theView = get_objectFromUserdata(__bridge HSWebViewView, L, 1, USERDATA_TAG) ;
...
/// hs._asm.enclosure.webview.datastore:fetchRecords([dataTypes], callback) -> datastoreObject
/// Method
/// Generates a list of the datastore records of the specified type, and invokes the callback function with the list.
///
/// Parameters:
///  * `dataTypes` - an optional string or table specifying the data types to fetch from the datastore.  If this parameter is not specified, it defaults to the list returned by [hs._asm.enclosure.webview.datastore.websiteDataTypes](#websiteDataTypes).
...
/// hs._asm.enclosure.webview.datastore:removeRecordsFor(displayNames, dataTypes, [callback]) -> datastoreObject
/// Method
...
///
/// Notes:
///  * to specify that all data types that qualify should be removed, specify the function  [hs._asm.enclosure.webview.datastore.websiteDataTypes()](#websiteDataTypes). as the second argument.
...
/// hs._asm.enclosure.webview.datastore:removeRecordsAfter(date, dataTypes, [callback]) -> datastoreObject
/// Method
...
/// ~~~
/// hs._asm.enclosure.webview.datastore.default():removeRecordsAfter(0, hs._asm.enclosure.webview.datastore.websiteDataTypes())
/// ~~~
...
/// hs._asm.enclosure.webview.datastore:persistent() -> bool
/// Method
...
///
/// Notes:
///  * Note that this value is the inverse of `hs._asm.enclosure.webview:privateBrowsing()`, since private browsing uses a non-persistent datastore.
...
int luaopen_hs__asm_enclosure_webview_datastore(lua_State* L) {
    LuaSkin *skin = [LuaSkin shared] ;
