...
#define USERDATA_TAG "hs._asm.enclosure.progress"
...
/// hs._asm.enclosure.progress:start() -> progressObject
/// Method
...
/// hs._asm.enclosure.progress:stop() -> progressObject
/// Method
...
/// hs._asm.enclosure.progress:threaded([flag]) -> progressObject | current value
/// Method
...
/// hs._asm.enclosure.progress:indeterminate([flag]) -> progressObject | current value
/// Method
...
/// Notes:
///  * The default setting for this is true.
///  * If this setting is set to false, you should also take a look at [hs._asm.enclosure.progress:min](#min) and [hs._asm.enclosure.progress:max](#max), and periodically update the status with [hs._asm.enclosure.progress:value](#value) or [hs._asm.enclosure.progress:increment](#increment)
...
/// hs._asm.enclosure.progress:bezeled([flag]) -> progressObject | current value
/// Method
...
/// hs._asm.enclosure.progress:visibleWhenStopped([flag]) -> progressObject | current value
/// Method
...
/// hs._asm.enclosure.progress:circular([flag]) -> progressObject | current value
/// Method
...
/// hs._asm.enclosure.progress:value([value]) -> progressObject | current value
/// Method
...
///  * For a determinate indicator, this will affect how "filled" the bar or circle is.  If the value is lower than [hs._asm.enclosure.progress:min](#min), then it will be reset to that value.  If the value is greater than [hs._asm.enclosure.progress:max](#max), then it will be reset to that value.
...
/// hs._asm.enclosure.progress:min([value]) -> progressObject | current value
/// Method
...
///  * The default value for this is 0.0
///  * This value has no effect on the display of an indeterminate progress indicator.
///  * For a determinate indicator, the behavior is undefined if this value is greater than [hs._asm.enclosure.progress:max](#max).
...
/// hs._asm.enclosure.progress:max([value]) -> progressObject | current value
/// Method
...
///  * This value has no effect on the display of an indeterminate progress indicator.
///  * For a determinate indicator, the behavior is undefined if this value is less than [hs._asm.enclosure.progress:min](#min).
...
/// hs._asm.enclosure.progress:increment(value) -> progressObject | current value
/// Method
...
///
/// Notes:
///  * Programmatically, this is equivalent to `hs._asm.enclosure.progress:value(hs._asm.enclosure.progress:value() + value)`, but is faster.
...
/// hs._asm.enclosure.progress:tint([tint]) -> progressObject | current value
/// Method
...
/// hs._asm.enclosure.progress:indicatorSize([size]) -> progressObject | current value
/// Method
...
/// hs._asm.enclosure.progress:color(color) -> progressObject
/// Method
...
///  * Because the filter must be applied differently depending upon the progress indicator style, make sure to invoke this method *after* [hs._asm.enclosure.progress:circular](#circular).
...
int luaopen_hs__asm_enclosure_progress_internal(lua_State* __unused L) {
    LuaSkin *skin = [LuaSkin shared]  ;
