...
#define USERDATA_TAG  "hs._asm.enclosure.toolbar"
...
@interface ASMToolbar : NSToolbar <NSToolbarDelegate>
...
- (instancetype)initWithCopy:(ASMToolbar *)original {
    LuaSkin *skin = [LuaSkin shared] ;
...
// this is the only way to switch a grouped set's individual labels back on after turning them off by setting a group label... and while the SDK says these aren't nullable, testing shows otherwise, so...
...
#pragma clang diagnostic pop
            ...
            if ([item isKindOfClass:[NSToolbarItemGroup class]] && !inGroup) {
                ...
                if (inGroup) {
                    ...
                    [skin logWarn:[NSString stringWithFormat:@"%s:cannot change currently visible toolbar item %@ type. Remove item from toolbar first.", USERDATA_TAG, identifier]] ;
                }
                [itemDefinition removeObjectForKey:keyName] ;
            }
            ...
            if ([keyValue isKindOfClass:[NSArray class]]) {
                ...
                if ([keyValue isKindOfClass:[NSNumber class]] && ![keyValue boolValue]) {
                    ...
                    [skin logWarn:[NSString stringWithFormat:@"%s:%@ for %@ must be an array, or false to remove", USERDATA_TAG, keyName, identifier]] ;
                }
                [itemDefinition removeObjectForKey:keyName] ;
            }
...
- (void)searchCallback:(NSMenuItem *)sender {
    ...
    [(ASMToolbar *)_toolbarItem.toolbar performCallback:self] ;
}
...
/// hs._asm.enclosure.toolbar.new(toolbarName, [toolbarTable]) -> toolbarObject
/// Constructor
/// Creates a new toolbar for a enclosure or the console.
///
/// Parameters:
...
///
/// Notes:
///  * Toolbar names must be unique, but a toolbar may be copied with [hs._asm.enclosure.toolbar:copy](#copy) if you wish to attach it to multiple windows (_asm(enclosure or console).
///  * See [hs._asm.enclosure.toolbar:addItems](#addItems) for a description of the format for `toolbarTable`

static int newASMToolbar(lua_State *L) {
    LuaSkin *skin = [LuaSkin shared] ;
    ...
    if (![identifiersInUse containsObject:identifier]) {
        ASMToolbar *toolbar = [[ASMToolbar alloc] initWithIdentifier:identifier
                                                    itemTableIndex:idx] ;
        if (toolbar) {
...
/// hs._asm.enclosure.toolbar.attachToolbar([obj1], [obj2]) -> obj1
/// Function
/// Get or attach/detach a toolbar to the console or enclosure.
///
/// Parameters:
...
///  * if one argument is provided and it is a enclosureObject, this function will return the current toolbarObject for the enclosure, or nil if one is not attached.
///  * if two arguments are provided and the first is a enclosureObject and the second is a toolbarObject or nil, this function will attach or detach a toolbarObject to/from the enclosure.
///
/// Returns:
///  * if the function is used to attach/detach a toolbar, then the first object provided will be returned ; if this function is used to get the current toolbar object for a enclosure or the console, then the toolbarObject or nil will be returned.
///
/// Notes:
///  * This function is not expected to be used directly (though it can be) -- it is added to the `hs._asm.enclosure` object metatable so that it may be invoked as `hs._asm.enclosure:toolbar([toolbarObject | nil])` and to the `hs.console` module so that it may be invoked as `hs.console.toolbar([toolbarObject | nil])`.
///
...
static int attachToolbar(lua_State *L) {
    ...
    if (lua_gettop(L) == 0) {
        ...
        setToolbar = YES ;
    } else if (lua_gettop(L) == 1 && (lua_type(L, 1) == LUA_TUSERDATA) && luaL_testudata(L, 1, "hs._asm.enclosure")) {
        theWindow = get_objectFromUserdata(__bridge NSWindow, L, 1, "hs._asm.enclosure") ;
        newToolbar = nil ;
        setToolbar = NO ;
    } else if (lua_gettop(L) == 2 && (lua_type(L, 1) == LUA_TUSERDATA) && luaL_testudata(L, 1, "hs._asm.enclosure") && (lua_type(L, 2) == LUA_TNIL)) {
        theWindow = get_objectFromUserdata(__bridge NSWindow, L, 1, "hs._asm.enclosure") ;
        newToolbar = nil ;
        setToolbar = YES ;
    } else if (lua_gettop(L) == 2 && lua_type(L, 1) == LUA_TUSERDATA && luaL_testudata(L, 1, "hs._asm.enclosure") && (lua_type(L, 2) == LUA_TUSERDATA) && luaL_testudata(L, 2, USERDATA_TAG)) {
        theWindow = get_objectFromUserdata(__bridge NSWindow, L, 1, "hs._asm.enclosure") ;
        ...
        return luaL_error(L, "%s:attachToolbar requires an optional hs._asm.enclosure object and an %s object or nil", USERDATA_TAG, USERDATA_TAG) ;
    }

    ASMToolbar *oldToolbar = (ASMToolbar *)theWindow.toolbar ;
    if (setToolbar) {
        if (oldToolbar) {
            oldToolbar.visible = NO ;
            theWindow.toolbar = nil ;
            if ([oldToolbar isKindOfClass:[ASMToolbar class]]) oldToolbar.windowUsingToolbar = nil ;
        }
        if (newToolbar) {
        ...
        if ([oldToolbar isKindOfClass:[ASMToolbar class]]) {
...
/// hs._asm.enclosure.toolbar:isAttached() -> boolean
/// Method
...
static int isAttachedToWindow(lua_State *L) {
    ...
    ASMToolbar *toolbar = [skin toNSObjectAtIndex:1] ;
    lua_pushboolean(L, [toolbar isAttachedToWindow]) ;
    return 1;
}

/// hs._asm.enclosure.toolbar:copy() -> toolbarObject
/// Method
...
///
/// Returns:
///  * a copy of the toolbar which can be attached to another window (_asm(enclosure or console).
static int copyToolbar(lua_State *L) {
    LuaSkin *skin = [LuaSkin shared] ;
    [skin checkArgs:LS_TUSERDATA, USERDATA_TAG, LS_TBREAK] ;
    ASMToolbar *oldToolbar = [skin toNSObjectAtIndex:1] ;
    ASMToolbar *newToolbar = [[ASMToolbar alloc] initWithCopy:oldToolbar] ;
    if (newToolbar) {
...
/// hs._asm.enclosure.toolbar:setCallback(fn | nil) -> toolbarObject
/// Method
...
///
///  The function should expect three (four, if the item is a `searchfield` or `notifyOnChange` is true) arguments and return none: the toolbar object, "console" or the enclosure object the toolbar is attached to, and the toolbar item identifier that was clicked.
/// Returns:
///  * the toolbar object.
///
/// Notes:
...
///  * if [hs._asm.enclosure.toolbar:notifyOnChange](#notifyOnChange) is set to true, then this callback function will also be invoked when a toolbar item is added or removed from the toolbar either programmatically with [hs._asm.enclosure.toolbar:insertItem](#insertItem) and [hs._asm.enclosure.toolbar:removeItem](#removeItem) or under user control with [hs._asm.enclosure.toolbar:customizePanel](#customizePanel) and the callback function will receive a string of "add" or "remove" as a fourth argument.
...
/// hs._asm.enclosure.toolbar:savedSettings() -> table
/// Method
/// Returns a table containing the settings which will be saved for the toolbar if [hs._asm.enclosure.toolbar:autosaves](#autosaves) is true.
///
/// Parameters:
///  * None
///
...
/// hs._asm.enclosure.toolbar:separator([bool]) -> toolbarObject | bool
/// Method
...
/// hs._asm.enclosure.toolbar:visible([bool]) -> toolbarObject | bool
/// Method
...
/// hs._asm.enclosure.toolbar:notifyOnChange([bool]) -> toolbarObject | bool
/// Method
...
/// hs._asm.enclosure.toolbar:insertItem(id, index) -> toolbarObject
/// Method
...
/// hs._asm.enclosure.toolbar:sizeMode([size]) -> toolbarObject
/// Method
...
/// hs._asm.enclosure.toolbar:displayMode([mode]) -> toolbarObject
/// Method
...
/// hs._asm.enclosure.toolbar:modifyItem(table) -> toolbarObject
/// Method
...
///  * For a list of the possible toolbar item attribute keys, see [hs._asm.enclosure.toolbar:addItems](#addItems).
...
static int addToolbarItems(lua_State *L) {
    ...
    while (isGood && (index < count)) {
        if (lua_rawgeti(L, 2, index + 1) == LUA_TTABLE) {
            ...
            [skin logWarn:[NSString stringWithFormat:@"%s:addItems - not a table at index %lld", USERDATA_TAG, index + 1]] ;
            isGood = NO ;
        }
        lua_pop(L, 1) ;
        index++ ;
    ...
    if (!isGood) {
        return luaL_error(L, "%s:addItems - malformed toolbar items encountered", USERDATA_TAG) ;
    } else {
        lua_pushvalue(L, 1) ;
    }
    return 1 ;
...
/// hs._asm.enclosure.toolbar:deleteItem(identifier) -> toolbarObject
/// Method
...
/// hs._asm.enclosure.toolbar:itemDetails(id) -> table
/// Method
...
///
/// Notes:
///  * For a list of the most of the possible toolbar item attribute keys, see [hs._asm.enclosure.toolbar:addItems](#addItems).
...
/// hs._asm.enclosure.toolbar:allowedItems() -> array
/// Method
...
///
/// Returns:
///  * a table as an array of all toolbar item identifiers defined for this toolbar.  See also [hs._asm.enclosure.toolbar:items](#items) and [hs._asm.enclosure.toolbar:visibleItems](#visibleItems).
...
/// hs._asm.enclosure.toolbar:items() -> array
/// Method
...
///
/// Returns:
///  * a table as an array of the currently active (assigned) toolbar item identifiers.  Toolbar items which are in the overflow menu *are* included in this array.  See also [hs._asm.enclosure.toolbar:visibleItems](#visibleItems) and [hs._asm.enclosure.toolbar:allowedItems](#allowedItems).
...
/// hs._asm.enclosure.toolbar:visibleItems() -> array
/// Method
...
///
/// Returns:
///  * a table as an array of the currently visible toolbar item identifiers.  Toolbar items which are in the overflow menu are *not* included in this array.  See also [hs._asm.enclosure.toolbar:items](#items) and [hs._asm.enclosure.toolbar:allowedItems](#allowedItems).
...
/// hs._asm.enclosure.toolbar:selectedItem([item]) -> toolbarObject | item
/// Method
...
///
/// Notes:
///  * Only toolbar items which were defined as `selectable` when created with [hs._asm.enclosure.toolbar.new](#new) can be selected with this method.
...
/// hs._asm.enclosure.toolbar:identifier() -> identifier
/// Method
...
/// hs._asm.enclosure.toolbar:customizePanel() -> toolbarObject
/// Method
...
/// hs._asm.enclosure.toolbar:isCustomizing() -> bool
/// Method
...
/// hs._asm.enclosure.toolbar:canCustomize([bool]) -> toolbarObject | bool
/// Method
...
///
/// Notes:
///  * the customization panel can be pulled up by right-clicking on the toolbar or by invoking [hs._asm.enclosure.toolbar:customizePanel](#customizePanel).
...
/// hs._asm.enclosure.toolbar:autossaves([bool]) -> toolbarObject | bool
/// Method
...
#ifdef _WK_DEBUG
// /// hs._asm.enclosure.toolbar:infoDump() -> table
// /// Method
...
// ///
// /// Returns:
// ///  * a table containing information stored in the ASMToolbar object for debugging purposes.
...
/// hs._asm.enclosure.toolbar.systemToolbarItems
/// Constant
...
/// hs._asm.enclosure.toolbar.itemPriorities
/// Constant
...
static int pushASMToolbar(lua_State *L, id obj) {
    LuaSkin *skin = [LuaSkin shared] ;
    ASMToolbar *value = obj;
    if (value.selfRef == LUA_NOREF) {
        void** valuePtr = lua_newuserdata(L, sizeof(ASMToolbar *));
...
static id toASMToolbarFromLua(lua_State *L, int idx) {
    LuaSkin *skin = [LuaSkin shared] ;
    ASMToolbar *value ;
    if (luaL_testudata(L, idx, USERDATA_TAG)) {
        value = get_objectFromUserdata(__bridge ASMToolbar, L, idx, USERDATA_TAG) ;
...
//     lua_pushboolean(L, value.autovalidates) ; lua_setfield(L, -2, "autovalidates") ;
    ...
    if ([value.toolbar isKindOfClass:[ASMToolbar class]]) {
...
static int userdata_tostring(lua_State* L) {
    LuaSkin *skin = [LuaSkin shared] ;
    ASMToolbar *obj = [skin luaObjectAtIndex:1 toClass:"ASMToolbar"] ;
...
// so use luaL_testudata before the macro causes a lua error
    if (luaL_testudata(L, 1, USERDATA_TAG) && luaL_testudata(L, 2, USERDATA_TAG)) {
        LuaSkin *skin = [LuaSkin shared] ;
        ASMToolbar *obj1 = [skin luaObjectAtIndex:1 toClass:"ASMToolbar"] ;
        ASMToolbar *obj2 = [skin luaObjectAtIndex:2 toClass:"ASMToolbar"] ;
...
/// hs._asm.enclosure.toolbar:delete() -> none
/// Method
...
static int userdata_gc(lua_State* L) {
    LuaSkin *skin = [LuaSkin shared] ;
    ASMToolbar *obj = get_objectFromUserdata(__bridge_transfer ASMToolbar, L, 1, USERDATA_TAG) ;
    if (obj) {
...
int luaopen_hs__asm_enclosure_toolbar_internal(lua_State* L) {
    LuaSkin *skin = [LuaSkin shared] ;
...
//     keysToKeepFromGroupDefinition      = @[ @"searchfield", @"image", @"fn" ];
    ...
    [skin registerPushNSHelper:pushASMToolbar         forClass:"ASMToolbar"];
    [skin registerLuaObjectHelper:toASMToolbarFromLua forClass:"ASMToolbar" withUserdataMapping:USERDATA_TAG];
