...
local enclosureMT = hs.getObjectMetatable(USERDATA_TAG)
...
enclosureMT._styleMask = enclosureMT.styleMask -- save raw version
enclosureMT.styleMask = function(self, ...) -- add nice wrapper version
    local arg = table.pack(...)
    local theMask = enclosureMT._styleMask(self)
    ...
    if arg.n ~= 0 then
        ...
        return enclosureMT._styleMask(self, theMask)
    else
        return theMask
    end
end

enclosureMT._collectionBehavior = enclosureMT.collectionBehavior -- save raw version
enclosureMT.collectionBehavior = function(self, ...)          -- add nice wrapper version
    local arg = table.pack(...)
    local theBehavior = enclosureMT._collectionBehavior(self)
    ...
    if arg.n ~= 0 then
        ...
        return enclosureMT._collectionBehavior(self, theBehavior)
    else
        return theBehavior
    end
end

enclosureMT._level = enclosureMT.level     -- save raw version
enclosureMT.level = function(self, ...) -- add nice wrapper version
    local arg = table.pack(...)
    local theLevel = enclosureMT._level(self)
    ...
    if arg.n ~= 0 then
        ...
        return enclosureMT._level(self, theLevel)
    else
        return theLevel
    end
end

enclosureMT.bringToFront = function(self, ...)
...
enclosureMT.sendToBack = function(self, ...)
    local args = table.pack(...)
...
enclosureMT.isVisible = function(self, ...) return not self:isOccluded(...) end
-- enclosureMT.toolbar   = module.toolbar.attachToolbar -- need to wait until webview/toolbar migrated into enclosure submodules

-- experiment to forward methods to view... may ease transition once canvas and webview are migrated into enclosure submodules
enclosureMT.forwardMethods = function(self, ...)
...
enclosureMT.__index = function(self, key)
    if enclosureMT[key] then
        return enclosureMT[key]
    else
