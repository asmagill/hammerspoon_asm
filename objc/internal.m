...
// FIXME: This needs to change, but to prevent crashes related to missing args for now...
    ...
    @try { // probably only useful for exceptions thrown withen an actual message, but anything which reduces crashes is a net positive...

        if (returnType[typePos] == '@') {           // ID
// TODO: for some reason sticking an ID in the switch/case below pisses off ARC...
            id result ;
            ffi_call(&cif, (callSuper ? FFI_FN(objc_msgSendSuper) : FFI_FN(objc_msgSend)), &result, values);
            push_object(L, result) ;
// FIXME: and these crash in the switch/case below...
        } else if (returnType[typePos] == '#') {    // Class
            Class result ;
            ffi_call(&cif, (callSuper ? FFI_FN(objc_msgSendSuper) : FFI_FN(objc_msgSend)), &result, values);
            push_class(L, result) ;
        } else if (returnType[typePos] == ':') {    // SEL
            SEL result ;
            ffi_call(&cif, (callSuper ? FFI_FN(objc_msgSendSuper) : FFI_FN(objc_msgSend)), &result, values);
            push_selector(L, result) ;
        } else {
            void **resultPtr ;
            ffi_call(&cif, (callSuper ? FFI_FN(objc_msgSendSuper) : FFI_FN(objc_msgSend)), resultPtr, values);
            ...
            switch(returnType[typePos]) {
                ...
                case 'C': {                         // unsigned char
                    unsigned char result = (unsigned char)*resultPtr ;
                    lua_pushinteger(L, result) ;
                    break ;
                }
                case 'i': {                         // int
                ...
                case 'l': {                         // long
                    long result = (long)*resultPtr ;
                    lua_pushinteger(L, result) ;
                    break ;
                }
                case 'q':                           // long long
                case 'Q': {                         // unsigned long long (lua can't do unsigned long long; choose bits over magnitude)
                    long long result = (long long)*resultPtr ;
                    lua_pushinteger(L, result) ;
                    break ;
                }
                case 'I': {                         // unsigned int
                    unsigned int result = (unsigned int)*resultPtr ;
                    lua_pushinteger(L, result) ;
                    break ;
                }
                case 'S': {                         // unsigned short
                    unsigned short result = (unsigned short)*resultPtr ;
                    lua_pushinteger(L, result) ;
                    break ;
                }
                case 'L': {                         // unsigned long
                    unsigned long result = (unsigned long)*resultPtr ;
                ...
                case 'f': {                         // float
                    float result ;
                    memcpy(&result, resultPtr, sizeof(float)) ;
                    lua_pushnumber(L, result) ;
                    break ;
                }
                case 'd': {                         // double
                    double result ;
                    memcpy(&result, resultPtr, sizeof(double)) ;
                    lua_pushnumber(L, result) ;
                    break ;
                }
...
//                 }
//                 case '#': {    // Class
//                     Class result  = (__bridge Class)(*resultPtr) ;
//                     push_class(L, result) ;
//                     break ;
//                 }
//                 case ':': {    // SEL
//                     SEL result = (SEL)(*resultPtr) ;
//                     push_selector(L, result) ;
//                     break ;
//                 }
