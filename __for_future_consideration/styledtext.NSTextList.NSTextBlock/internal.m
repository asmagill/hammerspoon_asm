// NOTE: Not useful at this time
//
// This version contains support for NSTextList and NSTextBlock which are apparently ignored
// the methods used by hs.canvas and hs.drawing but might be useful if I ever complete enclosure
// and support for NSTextContainer subviews.
...
static int font_convertFont(lua_State *L) {
    ...
    if (lua_type(L, 2) == LUA_TNUMBER) {
        [skin pushNSObject:[[NSFontManager sharedFontManager] convertFont:theFont toHaveTrait:(NSFontTraitMask)(luaL_checkinteger(L, 2))]];
    } else {
...
static int fontNamesWithTraits(lua_State *L) {
    ...
    NSFontTraitMask theTraits = (NSFontTraitMask)0;
...
static int NSTextList_toLua(lua_State *L, id obj) {
    LuaSkin *skin = [LuaSkin shared] ;
    NSTextList *list = obj ;
    lua_newtable(L) ;
    [skin pushNSObject:list.markerFormat] ; lua_setfield(L, -2, "format") ;
    lua_pushboolean(L, ((list.listOptions & NSTextListPrependEnclosingMarker) > 0)) ;
    lua_setfield(L, -2, "prepend") ;
    lua_pushinteger(L, list.startingItemNumber) ; lua_setfield(L, -2, "start") ;
    lua_pushstring(L, "NSTextList") ; lua_setfield(L, -2, "__luaSkinType") ;
    return 1 ;
}

static id table_toNSTextList(lua_State *L, int idx) {
    LuaSkin *skin = [LuaSkin shared];
    NSTextList *list ;
    if (lua_type(L, idx) == LUA_TTABLE) {
        NSString *format ;
        NSUInteger mask = 0 ;
        NSArray *tokens = @[ @"box", @"check", @"circle", @"diamond", @"disc", @"hyphen", @"square", @"lower-hexadecimal", @"upper-hexadecimal", @"octal", @"lower-alpha", @"lower-latin", @"upper-alpha", @"upper-latin", @"lower-roman", @"upper-roman", @"decimal" ];
        if (lua_getfield(L, idx, "format") == LUA_TSTRING) {
            format = [skin toNSObjectAtIndex:-1] ;
            BOOL isGood = NO ;
            for (NSString *token in tokens) {
                if ([format containsString:[NSString stringWithFormat:@"{%@}", token]]) {
                    // only one token is allowed in the string
                    if (isGood) {
                        isGood = NO ;
                        break ;
                    } else {
            ...
            if (!isGood) format = nil ;
        }
        lua_pop(L, 1) ;
        if (lua_getfield(L, idx, "prepend") == LUA_TBOOLEAN) {
            if (lua_toboolean(L, -1)) mask = NSTextListPrependEnclosingMarker ;
        }
        lua_pop(L, 1) ;
        if (format) {
            list = [[NSTextList alloc] initWithMarkerFormat:format options:mask] ;
            if ((lua_getfield(L, idx, "start") == LUA_TNUMBER) && lua_isinteger(L, -1)) {
                list.startingItemNumber = lua_tointeger(L, -1) ;
            }
            lua_pop(L, 1) ;
        } else {
            [skin logWarn:[NSString stringWithFormat:@"textListFormat must contain only one of {%@}", [tokens componentsJoinedByString:@"}, {"]]] ;
        }
    } else {
        [skin logAtLevel:LS_LOG_WARN
             withMessage:[NSString stringWithFormat:@"invalid textList object: expected table, found %s", lua_typename(L, lua_type(L, idx))]
            fromStackPos:1] ;
    }
    return list;
}

static int pushTextBlockTypeString(lua_State *L, NSTextBlockValueType type) {
    LuaSkin *skin = [LuaSkin shared] ;
    switch(type) {
        ...
        case NSTextBlockPercentageValueType:
            ...
            break ;
        default:
            [skin pushNSObject:[NSString stringWithFormat:@"unknown (%ld)", type]] ;
            break ;
    }
    return 1 ;
}

static NSTextBlockValueType TextBlockTypeFromString(lua_State *L, int idx) {
    LuaSkin *skin = [LuaSkin shared] ;
    NSTextBlockValueType result = NSTextBlockAbsoluteValueType ;
    if (lua_type(L, idx) == LUA_TSTRING) {
        NSString *value = [skin toNSObjectAtIndex:idx] ;
        if ([value isEqualToString:@"absolute"]) {
            result = NSTextBlockAbsoluteValueType ;
        } else if ([value isEqualToString:@"percentage"]) {
            result = NSTextBlockPercentageValueType ;
        } else {
            [skin logWarn:[NSString stringWithFormat:@"invalid textBlock value type: expected absolute or percentage, found %@", value]] ;
        }
    } else {
        [skin logWarn:[NSString stringWithFormat:@"invalid textBlock value type: expected string, found %s", lua_typename(L, lua_type(L, idx))]] ;
    }
    return result ;
}

static int NSTextBlock_toLua(lua_State *L, id obj) {
    LuaSkin *skin = [LuaSkin shared] ;
    NSTextBlock *block = obj ;
    lua_newtable(L) ;
    switch(block.verticalAlignment) {
        case NSTextBlockTopAlignment:      lua_pushstring(L, "top") ; break ;
        case NSTextBlockMiddleAlignment:   lua_pushstring(L, "middle") ; break ;
        case NSTextBlockBottomAlignment:   lua_pushstring(L, "bottom") ; break ;
        case NSTextBlockBaselineAlignment: lua_pushstring(L, "baseline") ; break ;
        default:
            [skin pushNSObject:[NSString stringWithFormat:@"unknown (%ld)", block.verticalAlignment]] ;
            break ;
    }
    lua_setfield(L, -2, "verticalAlignment") ;
    [skin pushNSObject:block.backgroundColor] ; lua_setfield(L, -2, "backgroundColor") ;
    lua_pushnumber(L, block.contentWidth) ; lua_setfield(L, -2, "contentWidth") ;
    pushTextBlockTypeString(L, block.contentWidthValueType) ;
    lua_setfield(L, -2, "contentWidthType") ;

    lua_newtable(L) ;
    lua_pushnumber(L, (lua_Number)[block valueForDimension:NSTextBlockWidth]) ;
    lua_setfield(L, -2, "width") ;
    lua_pushnumber(L, (lua_Number)[block valueForDimension:NSTextBlockMinimumWidth]) ;
    lua_setfield(L, -2, "minimumWidth") ;
    lua_pushnumber(L, (lua_Number)[block valueForDimension:NSTextBlockMaximumWidth]) ;
    lua_setfield(L, -2, "maximumWidth") ;
    lua_pushnumber(L, (lua_Number)[block valueForDimension:NSTextBlockHeight]) ;
    lua_setfield(L, -2, "height") ;
    lua_pushnumber(L, (lua_Number)[block valueForDimension:NSTextBlockMinimumHeight]) ;
    lua_setfield(L, -2, "minimumHeight") ;
    lua_pushnumber(L, (lua_Number)[block valueForDimension:NSTextBlockMaximumHeight]) ;
    lua_setfield(L, -2, "maximumHeight") ;
    pushTextBlockTypeString(L, [block valueTypeForDimension:NSTextBlockWidth]) ;
    lua_setfield(L, -2, "widthType") ;
    pushTextBlockTypeString(L, [block valueTypeForDimension:NSTextBlockMinimumWidth]) ;
    lua_setfield(L, -2, "minimumWidthType") ;
    pushTextBlockTypeString(L, [block valueTypeForDimension:NSTextBlockMaximumWidth]) ;
    lua_setfield(L, -2, "maximumWidthType") ;
    pushTextBlockTypeString(L, [block valueTypeForDimension:NSTextBlockHeight]) ;
    lua_setfield(L, -2, "heightType") ;
    pushTextBlockTypeString(L, [block valueTypeForDimension:NSTextBlockMinimumHeight]) ;
    lua_setfield(L, -2, "minimumHeightType") ;
    pushTextBlockTypeString(L, [block valueTypeForDimension:NSTextBlockMaximumHeight]) ;
    lua_setfield(L, -2, "maximumHeightType") ;
    lua_setfield(L, -2, "dimensions") ;

    lua_newtable(L) ;
    [skin pushNSObject:[block borderColorForEdge:NSRectEdgeMinX]] ; lua_setfield(L, -2, "minX") ;
    [skin pushNSObject:[block borderColorForEdge:NSRectEdgeMinY]] ; lua_setfield(L, -2, "minY") ;
    [skin pushNSObject:[block borderColorForEdge:NSRectEdgeMaxX]] ; lua_setfield(L, -2, "maxX") ;
    [skin pushNSObject:[block borderColorForEdge:NSRectEdgeMaxY]] ; lua_setfield(L, -2, "maxY") ;
    lua_setfield(L, -2, "borderColors") ;

    lua_newtable(L) ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockPadding edge:NSRectEdgeMinX]) ;
    lua_setfield(L, -2, "minXWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockPadding edge:NSRectEdgeMinX]) ;
    lua_setfield(L, -2, "minXWidthType") ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockPadding edge:NSRectEdgeMinY]) ;
    lua_setfield(L, -2, "minYWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockPadding edge:NSRectEdgeMinY]) ;
    lua_setfield(L, -2, "minYWidthType") ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockPadding edge:NSRectEdgeMaxX]) ;
    lua_setfield(L, -2, "maxXWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockPadding edge:NSRectEdgeMaxX]) ;
    lua_setfield(L, -2, "maxXWidthType") ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockPadding edge:NSRectEdgeMaxY]) ;
    lua_setfield(L, -2, "maxYWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockPadding edge:NSRectEdgeMaxY]) ;
    lua_setfield(L, -2, "maxYWidthType") ;
    lua_setfield(L, -2, "padding") ;

    lua_newtable(L) ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockBorder edge:NSRectEdgeMinX]) ;
    lua_setfield(L, -2, "minXWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockBorder edge:NSRectEdgeMinX]) ;
    lua_setfield(L, -2, "minXWidthType") ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockBorder edge:NSRectEdgeMinY]) ;
    lua_setfield(L, -2, "minYWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockBorder edge:NSRectEdgeMinY]) ;
    lua_setfield(L, -2, "minYWidthType") ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockBorder edge:NSRectEdgeMaxX]) ;
    lua_setfield(L, -2, "maxXWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockBorder edge:NSRectEdgeMaxX]) ;
    lua_setfield(L, -2, "maxXWidthType") ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockBorder edge:NSRectEdgeMaxY]) ;
    lua_setfield(L, -2, "maxYWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockBorder edge:NSRectEdgeMaxY]) ;
    lua_setfield(L, -2, "maxYWidthType") ;
    lua_setfield(L, -2, "border") ;

    lua_newtable(L) ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockMargin edge:NSRectEdgeMinX]) ;
    lua_setfield(L, -2, "minXWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockMargin edge:NSRectEdgeMinX]) ;
    lua_setfield(L, -2, "minXWidthType") ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockMargin edge:NSRectEdgeMinY]) ;
    lua_setfield(L, -2, "minYWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockMargin edge:NSRectEdgeMinY]) ;
    lua_setfield(L, -2, "minYWidthType") ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockMargin edge:NSRectEdgeMaxX]) ;
    lua_setfield(L, -2, "maxXWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockMargin edge:NSRectEdgeMaxX]) ;
    lua_setfield(L, -2, "maxXWidthType") ;
    lua_pushnumber(L, (lua_Number)[block widthForLayer:NSTextBlockMargin edge:NSRectEdgeMaxY]) ;
    lua_setfield(L, -2, "maxYWidth") ;
    pushTextBlockTypeString(L, [block widthValueTypeForLayer:NSTextBlockMargin edge:NSRectEdgeMaxY]) ;
    lua_setfield(L, -2, "maxYWidthType") ;
    lua_setfield(L, -2, "margin") ;

    lua_pushstring(L, "NSTextBlock") ; lua_setfield(L, -2, "__luaSkinType") ;
    return 1 ;
}

static id table_toNSTextBlock(lua_State *L, int idx) {
    LuaSkin *skin = [LuaSkin shared] ;
    NSTextBlock *block ;
    if (lua_type(L, idx) == LUA_TTABLE) {

        block = [[NSTextBlock alloc] init] ;

        if (lua_getfield(L, idx, "verticalAlignment") == LUA_TSTRING) {
        ...
        if (lua_getfield(L, idx, "backgroundColor") == LUA_TTABLE) {
            block.backgroundColor = [skin luaObjectAtIndex:-1 toClass:"NSColor"] ;
        }
        lua_pop(L, 1);

        if (lua_getfield(L, idx, "contentWidth") == LUA_TNUMBER) {
            CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
            NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
            if (lua_getfield(L, idx, "contentWidthType")) type = TextBlockTypeFromString(L, -1) ;
            lua_pop(L, 1) ;
            [block setContentWidth:width type:type] ;
        }
        lua_pop(L, 1) ;

        if (lua_getfield(L, idx, "borderColors") == LUA_TTABLE) {
            if (lua_getfield(L, -1, "all") == LUA_TTABLE) {
                [block setBorderColor:[skin luaObjectAtIndex:-1 toClass:"NSColor"]] ;
            }
            lua_pop(L, 1);
            if (lua_getfield(L, -1, "minX") == LUA_TTABLE) {
                [block setBorderColor:[skin luaObjectAtIndex:-1 toClass:"NSColor"] forEdge:NSRectEdgeMinX] ;
            }
            lua_pop(L, 1);
            if (lua_getfield(L, -1, "minY") == LUA_TTABLE) {
                [block setBorderColor:[skin luaObjectAtIndex:-1 toClass:"NSColor"] forEdge:NSRectEdgeMinY] ;
            }
            lua_pop(L, 1);
            if (lua_getfield(L, -1, "maxX") == LUA_TTABLE) {
                [block setBorderColor:[skin luaObjectAtIndex:-1 toClass:"NSColor"] forEdge:NSRectEdgeMaxX] ;
            }
            lua_pop(L, 1);
            if (lua_getfield(L, -1, "maxY") == LUA_TTABLE) {
                [block setBorderColor:[skin luaObjectAtIndex:-1 toClass:"NSColor"] forEdge:NSRectEdgeMaxY] ;
            }
            lua_pop(L, 1);
        }
        lua_pop(L, 1) ;

        if (lua_getfield(L, idx, "dimensions") == LUA_TTABLE) {
            if (lua_getfield(L, -1, "width") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "widthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setValue:width type:type forDimension:NSTextBlockWidth] ;
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "minimumWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "minimumWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setValue:width type:type forDimension:NSTextBlockMinimumWidth] ;
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "maximumWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "maximumWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setValue:width type:type forDimension:NSTextBlockMaximumWidth] ;
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "height") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "heightType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setValue:width type:type forDimension:NSTextBlockHeight] ;
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "minimumHeight") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "minimumHeightType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setValue:width type:type forDimension:NSTextBlockMinimumHeight] ;
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "maximumHeight") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "maximumHeightType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setValue:width type:type forDimension:NSTextBlockMaximumHeight] ;
            }
            lua_pop(L, 1) ;
        }
        lua_pop(L, 1) ;

        if (lua_getfield(L, idx, "padding") == LUA_TTABLE) {
            if (lua_getfield(L, -1, "width") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "widthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockPadding];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "minXWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "minXWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockPadding edge:NSRectEdgeMinX];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "minYWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "minYWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockPadding edge:NSRectEdgeMinY];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "maxXWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "maxXWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockPadding edge:NSRectEdgeMaxX];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "maxYWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "maxYWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockPadding edge:NSRectEdgeMaxY];
            }
            lua_pop(L, 1) ;
        }
        lua_pop(L, 1) ;

        if (lua_getfield(L, idx, "border") == LUA_TTABLE) {
            if (lua_getfield(L, -1, "width") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "widthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockBorder];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "minXWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                ...
                if (lua_getfield(L, -2, "minXWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockBorder edge:NSRectEdgeMinX];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "minYWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                ...
                if (lua_getfield(L, -2, "minYWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockBorder edge:NSRectEdgeMinY];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "maxXWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                ...
                if (lua_getfield(L, -2, "maxXWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockBorder edge:NSRectEdgeMaxX];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "maxYWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                ...
                if (lua_getfield(L, -2, "maxYWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockBorder edge:NSRectEdgeMaxY];
            }
            lua_pop(L, 1) ;
        }
        lua_pop(L, 1) ;

        if (lua_getfield(L, idx, "margin") == LUA_TTABLE) {
            if (lua_getfield(L, -1, "width") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                NSTextBlockValueType type = NSTextBlockAbsoluteValueType ;
                if (lua_getfield(L, -2, "widthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockMargin];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "minXWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                ...
                if (lua_getfield(L, -2, "minXWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockMargin edge:NSRectEdgeMinX];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "minYWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                ...
                if (lua_getfield(L, -2, "minYWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockMargin edge:NSRectEdgeMinY];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "maxXWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                ...
                if (lua_getfield(L, -2, "maxXWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockMargin edge:NSRectEdgeMaxX];
            }
            lua_pop(L, 1) ;
            if (lua_getfield(L, -1, "maxYWidth") == LUA_TNUMBER) {
                CGFloat width = (CGFloat)lua_tonumber(L, -1) ;
                ...
                if (lua_getfield(L, -2, "maxYWidthType")) type = TextBlockTypeFromString(L, -1) ;
                lua_pop(L, 1) ;
                [block setWidth:width type:type forLayer:NSTextBlockMargin edge:NSRectEdgeMaxY];
            }
            lua_pop(L, 1) ;
        }
        lua_pop(L, 1) ;
        ...
        [skin logAtLevel:LS_LOG_WARN
             withMessage:[NSString stringWithFormat:@"invalid textBlock object: expected table, found %s", lua_typename(L, lua_type(L, idx))]
            fromStackPos:1] ;
    }
    return block ;
}
...
#pragma clang diagnostic pop
    ...
    [skin pushNSObject:[thePS textLists]];
    lua_setfield(L, -2, "textLists");
    [skin pushNSObject:thePS.textBlocks];
    lua_setfield(L, -2, "textBlocks");
    ...
    if (lua_type(L, idx) == LUA_TTABLE) {
        ...
        if (lua_getfield(L, idx, "tabStops") == LUA_TTABLE) {
            ...
            while (lua_rawgeti(L, -1, pos) != LUA_TNIL) {
                if (lua_type(L, -1) == LUA_TTABLE) {
                    NSTextTab *textTab = [skin luaObjectAtIndex:-1 toClass:"NSTextTab"];
                    if (textTab) {
                        [theTabStops addObject:textTab];
                    } else {
                        [skin logWarn:[NSString stringWithFormat:@"invalid tabStop at index %lld", pos]] ;
                    }
        ...
        if (lua_getfield(L, idx, "textLists") == LUA_TTABLE) {
            NSMutableArray *textLists = [[NSMutableArray alloc] init] ;
            lua_Integer    pos        = 1 ;

            while(lua_rawgeti(L, -1, pos) != LUA_TNIL) {
                if (lua_type(L, -1) == LUA_TTABLE) {
                    NSTextList *textList = [skin luaObjectAtIndex:-1 toClass:"NSTextList"] ;
                    if (textList) {
                        [textLists addObject:textList] ;
                    } else {
                        [skin logWarn:[NSString stringWithFormat:@"invalid textList at index %lld", pos]] ;
                    }
                    lua_pop(L, 1) ; // the textList table we just looked at
                } else {
                    [skin logAtLevel:LS_LOG_WARN
                         withMessage:[NSString stringWithFormat:@"invalid textList at position %lld: expected table, found %s", pos, lua_typename(L, lua_type(L, -1))]
                        fromStackPos:1] ;
                }
                pos++;
            }
            lua_pop(L, 1) ; // loop terminating nil
            thePS.textLists = textLists ;
        }
        lua_pop(L, 1) ;

        if (lua_getfield(L, idx, "textBlocks") == LUA_TTABLE) {
            NSMutableArray *textBlocks = [[NSMutableArray alloc] init] ;
            lua_Integer    pos        = 1 ;

            while(lua_rawgeti(L, -1, pos) != LUA_TNIL) {
                if (lua_type(L, -1) == LUA_TTABLE) {
                    NSTextBlock *textBlock = [skin luaObjectAtIndex:-1 toClass:"NSTextBlock"] ;
                    if (textBlock) {
                        [textBlocks addObject:textBlock] ;
                    } else {
                        [skin logWarn:[NSString stringWithFormat:@"invalid textBlock at index %lld", pos]] ;
                    }
                    lua_pop(L, 1) ; // the textList table we just looked at
                } else {
                    [skin logAtLevel:LS_LOG_WARN
                         withMessage:[NSString stringWithFormat:@"invalid textBlock at position %lld: expected table, found %s", pos, lua_typename(L, lua_type(L, -1))]
                        fromStackPos:1] ;
                }
                pos++;
            }
            lua_pop(L, 1) ; // loop terminating nil
            thePS.textBlocks = textBlocks ;
        }
        lua_pop(L, 1) ;
...
int luaopen_hs_styledtext_internal(lua_State *L) {
    ...
    [skin registerPushNSHelper:NSTextList_toLua                forClass:"NSTextList"] ;
    [skin registerLuaObjectHelper:table_toNSTextList           forClass:"NSTextList"
                                                       withTableMapping:"NSTextList"];

    [skin registerPushNSHelper:NSTextBlock_toLua               forClass:"NSTextBlock"] ;
    [skin registerLuaObjectHelper:table_toNSTextBlock          forClass:"NSTextBlock"
                                                       withTableMapping:"NSTextBlock"];
