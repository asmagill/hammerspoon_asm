...
{
    if (self.occlusionState & NSWindowOcclusionStateVisible) [self display] ;
}
...
- (void)drawRect:(__unused NSRect)rect {
    if (!_skipOcclusionCheck && self.window && !([self.window occlusionState] & NSWindowOcclusionStateVisible)) return ;
