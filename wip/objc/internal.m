...
// NSLog(@"in object gc") ;
//     if (luaL_testudata(L, 1, ID_USERDATA_TAG)) {
// check to make sure we're not called with the wrong type for some reason...
        id __unused obj = get_objectFromUserdata(__bridge_transfer id, L, 1, ID_USERDATA_TAG) ;
//         id __unused obj = get_objectFromUserdata(__bridge id, L, 1, ID_USERDATA_TAG) ;
    #if defined(DEBUG_GC) || defined(DEBUG_GC_OBJONLY)
        NSLog(@"object: remove %p", obj) ;
    #endif

    // We may not be the only one holding on to this object, so just decrement our claim to it and
    // let ARC handle the rest...
    //    obj = nil ;
...
static const luaL_Reg object_userdata_metaLib[] = {
    {"_class",       objc_object_getClass},
    {"_className",   objc_object_getClassName},
    {"_value",       object_value},
...
// TODO TEST: can you call msgSendSuper when receiver is a class?
    ...
    NSLog(@"%s = %s with %d arguments", returnType, messageHolder, argCount) ;
...
// FIXME: This needs to change, but to prevent crashes related to missing args for now...
    ...
    @try {
        switch(returnType[typePos]) {
            ...
            case '@': {    // id
// 982	o = require("hs._asm.objc")
// 983	a = o.class("OBJCTest"):allocAndMsgSend("init")
// 984	a:returnFloat()
// 985	a:returnDouble()
// 986	a:returnNSString()
// 987	a:returnNSString()
// 988	a:returnNSString():_value()
// 989	inspect(a:propertyList())
// 990	inspect(a:_propertyList())
// 991	inspect(a:_propertyList(true))
// 992	inspect(a:_propertyValues(true))
// 993	collectgarbage()
// 994	o = require("hs._asm.objc")
// 995	a = o.class("OBJCTest"):allocAndMsgSend("init")
// 996	a:returnFloat()
// 997	a:returnDouble()
// 998	a:returnNSString()
// 999	a:returnNSString():_value()
// 1000	inspect(a:_propertyValues(true))
// 1001	collectgarbage()
//
// All crash at some point during this sequence or upon restart after this sequence.  Most promising lead
// was (from Console):
// Application Specific Information:
// objc[97603]: _NSTrackingRegionData object 0x7fdf458a9450 overreleased while already deallocating; break on objc_overrelease_during_dealloc_error to debug
// objc[97603]: __NSDictionaryM object 0x7fdf45ab3d40 overreleased while already deallocating; break on objc_overrelease_during_dealloc_error to debug
...
//                 ffi_call(&cif, (callSuper ? FFI_FN(objc_msgSendSuper) : FFI_FN(objc_msgSend)), &result, values);
//                 id result2 = result ;
//                 push_object(L, result2) ;
//                 id __autoreleasing *result ;
//                 ffi_call(&cif, (callSuper ? FFI_FN(objc_msgSendSuper) : FFI_FN(objc_msgSend)), result, values);
//                 push_object(L, *result) ;
                void **result ;
                ffi_call(&cif, (callSuper ? FFI_FN(objc_msgSendSuper) : FFI_FN(objc_msgSend)), result, values);
                id result2 = (__bridge id)*result ;
                push_object(L, result2) ;
                break ;
            }
