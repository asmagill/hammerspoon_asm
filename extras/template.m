...
// // inspect(_asm.extras.getMenuArray(hs.application.frontmostApplication()))
...
static void _buildMenuArray(lua_State* L, AXUIElementRef app, AXUIElementRef menuItem) {
    CFArrayRef cf_attributes;
    AXError error = AXUIElementCopyAttributeNames(menuItem, &cf_attributes);
    if (error) {
        NSLog(@"Unable to get attribute names: AXError %d", error) ;
        lua_pushfstring(L, "unable to get attribute names: AXError %d", error) ; lua_setfield(L, -2, "error") ;
    } else {
        NSMutableArray *attributeNames = [[NSMutableArray alloc] init];
        [attributeNames addObjectsFromArray:(__bridge NSArray *)cf_attributes];
        if ([attributeNames count] > 0) {
            lua_newtable(L) ;
            for(unsigned int i = 0 ; i < [attributeNames count] ; i++) {
                CFTypeRef cf_name = (__bridge CFTypeRef)[attributeNames objectAtIndex: i] ;
                NSString* ns_name = (__bridge_transfer NSString *)cf_name;

                CFIndex count = -1;
                error = AXUIElementGetAttributeValueCount(menuItem, cf_name, &count);
                if (error && error != kAXErrorInvalidUIElement) {
// This may be where we deal with final values...
                    NSLog(@"Unable to get count for %@: AXError %d", ns_name, error) ;
                    lua_pushfstring(L, "unable to get child count: AXError %d", error) ; lua_setfield(L, -2, "error") ;
                } else {
                    if (count > 0) {
                        CFArrayRef cf_children;
                        error = AXUIElementCopyAttributeValues(menuItem, cf_name, 0, count, &cf_children);
                        if (error) {
                            NSLog(@"Unable to get children for %@: AXError %d", ns_name, error) ;
...
//                            lua_newtable(L) ;
                            for(unsigned int j = 0 ; j < [toCheck count] ; j++) {
                                AXUIElementRef element = (__bridge AXUIElementRef)[toCheck objectAtIndex: j] ;
                                lua_newtable(L) ;
...
//                     } else if (count == 0) {
                        ...
                        CFTypeRef cf_value; error = AXUIElementCopyAttributeValue(menuItem, cf_name, &cf_value);
                        if (error) {
                            NSLog(@"Unable to get value for %@: AXError %d", ns_name, error) ;
                            lua_pushfstring(L, "unable to get value: AXError %d", error) ; lua_setfield(L, -2, "error") ;
                        } else {
                            NSString* ns_value = (__bridge_transfer NSString *)cf_value;
                            lua_pushstring(L, [ns_value UTF8String]) ;
                        }
                    }
                }
//                lua_setfield(L, -2, cf_name) ;
//                CFRelease(element) ;
            }
